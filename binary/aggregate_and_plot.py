...
def aggregate_and_plot(job_id, N_K, U, phi):
    ...
    for i, band_energies in enumerate(all_bound_state_bands):
        ax.plot(K_values / np.pi, band_energies, 'o-', color=colors[i % len(colors)],
                linewidth=2, markersize=4, label=f'Bound State Band #{i+1}')
    
    ax.set_xlabel(r'Center-of-Mass Momentum $K/\pi$')
    ax.set_ylabel('Energy $E$')
    ax.set_title(fr'Three-Particle Dispersion for $U={U}, \phi={phi:.2f}\pi$')
    ax.legend()
    ax.set_xlim(-1, 1)
    
    plt.savefig(f"final_bands_{U}U.pdf")
    print("Plot saved to final_bands.pdf")
    plt.show()

if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Aggregate and plot two-particle band structure data.")
