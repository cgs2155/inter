...
def H0(k_vec, phi):
    ...
    Accepts either a tuple k or a 1D array of tuples of kx and ky.
    Returns shape (N_SUB,N_SUB) for scalar input, or (N_k,N_SUB,N_SUB) for array input.
    """
    ...
    H = np.zeros((N, N_SUB, N_SUB), dtype=complex)
    for i, ki in enumerate(k_arr):
        kx = ki[0]
        ky = ki[1]
        H_i = np.zeros((N_SUB, N_SUB), dtype=complex)
        # use the i-th momentum value everywhere
        H_i[0, 1] = np.exp(-1j* kx) + w 
        H_i[0, 3] = np.exp(-1j * ( kx + ky)) + w_c * np.exp(-1j*kx)
        H_i[0, 4] = 1 + w_c * np.exp(-1j*ky)
        H_i[0, 5] = np.exp(-1j*ky) + w*np.exp(-1j*(kx+ky))
        H_i[1, 2] = w
        H_i[2, 3] = 1
        H_i[2, 4] = 1
        H_i[2, 5] = w_c
